...
class MovieSerializer(serializers.ModelSerializer):
    # ======== READ ========
    actors_info = MovieActorSerializer(
        source="movieactor_set", 
        many=True, 
        read_only=True
    )
    director_info = PersonSerializer(
        source="director", 
        read_only=True
    )
    genres_info = MovieGenreSerializer(
        source="moviegenre_set", 
        many=True, 
        read_only=True
    )

    # ======== WRITE ========
    actors = MovieActorSerializer(
        source="movieactor_set", 
        many=True, 
        write_only=True, 
        required=False
    ...
    def _update_m2m(self, instance, model, data_list):
        ...
        model.objects.bulk_create(
            [model(movie=instance, **item) for item in data_list]
        )
