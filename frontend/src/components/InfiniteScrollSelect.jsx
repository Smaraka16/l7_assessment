...
const InfiniteScrollSelect = ({
  ...
  const allOption = options.find(
    (opt) => opt.id === "" || opt.name.startsWith("All "),
  );
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      // Load more data when scrolled to 80% of the list
      if (
  ...
  const selectedOption = options.find(
    (opt) =>
      opt.name === value ||
      (value === "" && (opt.id === "" || opt.name.startsWith("All "))),
  );

  const handleSelect = (option) => {
    // If all option is selected, set value to empty string
    const valueToSet =
      option.id === "" || option.name.startsWith("All ") ? "" : option.name;
    onChange({
      target: {
        name,
        value: valueToSet,
  ...
  return (
    <div ref={dropdownRef} className={`relative ${className}`}>
      ...
      >
        <span
          className={selectedOption ? "truncate" : "text-white/50 truncate"}
        >
